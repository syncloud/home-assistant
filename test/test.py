...
def test_refresh(app_archive_path, device_host, device_session, device_password, domain):
    local_install(device_host, device_password, app_archive_path)
    wait_for_installer(device_session, domain)
...
def test_matter_bluetooth_decision_logged(device):
    device.run_ssh("sh -c 'journalctl -u snap.home-assistant.matter | grep -q bluetooth'", retries=50)
