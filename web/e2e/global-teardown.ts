...
function collectArtifacts () {
  ...
  for (const name of fs.readdirSync(resultsDir)) {
    ...
    const pngs = fs.readdirSync(dir).filter((f: string) => f.endsWith('.png')).sort()
    pngs.forEach((png: string, i: number) => {
      const suffix = i === 0 ? '' : `-${i}`
...
export default async function globalTeardown () {
  ...
  const journal = ssh('journalctl -u snap.home-assistant.server --no-pager | tail -800', { throw: false })
  fs.writeFileSync(path.join(artifactRoot, `home-assistant.${project}.journal.log`), journal)
  collectArtifacts()
}
